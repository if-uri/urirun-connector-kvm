# VENDORED COPY of urirun_cdp/cdp.py — do NOT edit here. Bundled so the kvm connector has a
# working CDP client on nodes that lack the urirun-cdp package (flat --code deploy). The kvm
# cdp.py shim prefers the installed urirun_cdp and falls back to this copy. A guard test
# (test_cdp_impl_vendor_matches) asserts this stays byte-identical to the source.
